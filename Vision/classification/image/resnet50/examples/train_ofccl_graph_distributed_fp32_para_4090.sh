...
export LD_LIBRARY_PATH=/HOME/scw6cab/run/OCCL/ofccl/build/lib:/data/apps/cuda/11.8/lib64:/data/apps/cudnn/8.7.0_cuda11.x/lib:/usr/lib64

export ONEFLOW_ENABLE_OFCCL=1
export ONEFLOW_OFCCL_SKIP_NEGO=0
export ONEFLOW_DEBUG_MODE=0
...
if [[ $DEVICE_NUM_PER_NODE -eq 2 ]]; then
...
elif [[ $DEVICE_NUM_PER_NODE -eq 4 ]]; then
    ...
    if [[ "$ONEFLOW_OFCCL_SKIP_NEGO" -eq "1" ]]; then
...
elif [[  $DEVICE_NUM_PER_NODE -eq 8 ]]; then
...
export OFRECORD_PATH=/HOME/scw6cab/run/OCCL/ImageNet
...
if [[ $ONEFLOW_ENABLE_OFCCL -eq 1 ]]; then
    cp /HOME/scw6cab/run/OCCL/oneflow/log/oneflow.log resnet_${HOST}_BASE_${BASE_CTX_SWITCH_THRESHOLD}_FACTOR_${RECV_SUCCESS_FACTOR}_UP_${RECV_SUCCESS_THRESHOLD}_TRYHEAD_${NUM_TRY_TASKQ_HEAD}.log
fi
